# **MY AI PROJECT**
This is a GitHub Repository that is a collection of my work on understanding and creating an AI. I am using **java** to create the AI algorithms, along aside a graph data structure to feed information to my creations.

---

## Types of AI Algorithms
1. Depth First Search
> "This List will be updated as I learn more algortithms and make their code"

---

## Depth First Search

<img width="390" height="250" alt="image" src="https://github.com/user-attachments/assets/9f920ac7-10d1-4cae-a41a-9a9b69050994" />

### ***Overview:*** 
A Depth First Search algorithm takes a graph of data and goes as far down as it can in one direction before back tracking untill it finds a new way to go.
+ PROS: At best, this Algorithm is the fastest if it "Lucks out" and always chooses the right path to the solution (by chance), then depth first search takes the least possible time to get to the solution.
+ CONS: It is possible that the found solution is not optimal. At worst this algorithm will explore every possible path before finding the solution, thus taking the longest possible time before reaching the solution.

---

## How to use
1. Add your data points using commands in `Graph.java`
2. Then call one of the searching methonds with a starting point

---

## Citations

+ [Havard Lecture Course](https://cs50.harvard.edu/ai/notes/0/)
+ [Depth First Search Visual Example](https://www.google.com/url?sa=t&source=web&rct=j&url=https%3A%2F%2Fen.wikipedia.org%2Fwiki%2FDepth-first_search&ved=0CBYQjRxqFwoTCKDg6NG7opQDFQAAAAAdAAAAABAe&opi=89978449)
